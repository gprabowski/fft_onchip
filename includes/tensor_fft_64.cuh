...
#pragma unroll
    for (int i = 0; i < ffts_per_unit; ++i) {
      local_b[0] = local_data[(indexing.brow * 2) * 8 + indexing.bcol];
      local_b[1] = local_data[(indexing.brow * 2 + 1) * 8 + indexing.bcol];
      ...
      local_data[i * Size + indexing.crow * 8 + indexing.ccol] = local_b[0];
